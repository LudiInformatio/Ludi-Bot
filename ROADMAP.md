...
**Last Updated:** February 24, 2026 — 7:48 PM EST
...
| Infra ✅ | Hybrid Off/Def Role Tagging | DONE | `players.archetype` = 15 offensive archetypes only (no defensive labels). `players.defensive_tag` = deterministic (PERIMETER_HAWK/RIM_GUARDIAN/SWITCHABLE_ANCHOR/HUSTLE_DISRUPTOR/WEAK_LINK/NULL). `_assign_defensive_tag()` in `classify_archetypes.py` — no Claude, pure threshold logic. Module X defensive guard removed (archetype always offensive). 23.0% GENERALIST ✅ | $0 |
| Infra ✅ | Scheme Cache d14 Fix + Quality Tiers | DONE | `team_scheme_cache.d14_style` fixed: self-referential subquery into `player_game_tracking` (was stale `games` JOIN). `off_quality_14d`/`def_quality_14d` STRONG/AVERAGE/WEAK via BDL `player_game_advanced` z-score tiers. All 30 teams now have real 14d tracking-based scheme labels. | $0 |
| Infra ✅ | Morning Brief Slate Trends Header | DONE | `_build_slate_trends_header()` in `morning_brief.py` — single Telegram message before game notes loop. HOT (L7 ≥+2.5) / COOLING (≤-2.5) players, injury-filtered (`OUT`/`DOUBTFUL` excluded), all slate teams. Combo props (PRA/PA/PR) surface when they're the dominant trend signal. | $0 |
