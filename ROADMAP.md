...
**Last Updated:** February 20, 2026 — 9:27 PM EST
...
**Completed:** Phases 5–7 ✅ + Phase 8.0-A/B/C/D ✅ + Phase 8.2/8.3/8.4/8.5/8.6/8.7/8.9/8.10/8.12/8.14/8.15 ✅ + Slack/Notification Split ✅ + Model Calibration Fixes ✅ + Feb 20 Post-ASB Audit ✅ + Tank01 Data Expansion ✅ + Injury Intelligence Hardening ✅ + Claude Auth Fix ✅ + Ask Ludi Architecture Research ✅ + Morning Brief Pipeline Hardening ✅ + BetIQ/TeamRankings Research ✅ + BERT/NLP Prompt Architecture Research ✅
...
| 8.19 | Prompt Engineering Upgrade | MEDIUM | Apply 8 BERT-derived patterns to Claude prompts. (1) Few-shot examples added to `GAME_NOTES_TEMPLATE` + `SPOTLIGHT_TEMPLATE` (zero-shot → 1-shot eliminates format drift). (2) Domain WR stats injected into Sonnet curation prompt (BLOCKS UNDER 63%, UNDER 55%, OVER 42%). (3) text_a/text_b section separation for historical vs tonight's context. (4) Label space defined first in all Haiku calls. (5) Haiku NSP news relevance gate replaces keyword matching in `_score_game()`. (6) Pre-truncate injected template blocks (prevent silent overflow). (7) Parse failure logging for audit. (8) Sonnet→Haiku feedback loop design (Phase 8.20). Full reference: `best-practices/ai/PROMPT_ENGINEERING_PATTERNS.md`. | ~$0.001/day |
