...
def get_all_games_in_range(start_date, end_date):
    ...
    c.execute('''
        SELECT game_id, date, nba_game_id FROM games
        WHERE date >= ? AND date <= ?
        ORDER BY date
    ''', (start_date, end_date))

    # Return tuples of (game_id, date, nba_game_id)
    games = [(row[0], row[1], row[2]) for row in c.fetchall()]
    conn.close()
...
def run_backfill(start_date, end_date):
    ...
    all_games_raw = get_all_games_in_range(start_date, end_date)
    print(f"\n[BACKFILL] Found {len(all_games_raw)} total games in range")

    # Resolve best game ID
    all_games = []
    for g_id, g_date, nba_id in all_games_raw:
        final_id = g_id
        if nba_id and len(str(nba_id)) == 10:
             final_id = str(nba_id)
        elif g_id and len(str(g_id)) == 8:
             final_id = "00" + str(g_id)
        
        all_games.append((final_id, g_date))
