"""
Phase 4: B2B Fatigue - 60-Day Backtest Validation

Analyzes fatigue adjustment performance using real game results from last 60 days.
...
Context:
60-day window provides ~6,000+ player-games for stronger statistical confidence.
Validates Phase A adjustments with larger sample sizes.
"""
...
class FatigueBacktest:
    ...
    def run_backtest(self):
        """Run 60-day backtest."""
        print(f"\n{'='*70}")
        print(f"  PHASE 4: B2B FATIGUE - 60-DAY BACKTEST")
        print(f"{'='*70}")
        print(f"\nAnalyzing games from last {self.days} days...")

        cursor = self.conn.cursor()

        # Get all player-games from last 60 days
        cursor.execute("""
            SELECT
                player_name,
                team_abbreviation,
    ...
    def print_report(self):
        ...
        print(f"\n{'='*70}")
        print(f"  FATIGUE ADJUSTMENT PERFORMANCE REPORT (60-DAY)")
        print(f"{'='*70}")
        ...
        if road_b2b_guard:
            ...
            print(f"   Modifier: -4.8% B2B Road, -2% Guard Tax = -6.7% total")
        ...
        if home_b2b_guard:
            ...
            print(f"   Modifier: -1.5% B2B Home, -2% Guard Tax = -3.5% total")
            print(f"   Mean Error: {metrics['mean_error']:+.2f} pts")
        ...
        density = self.results.get('Density_4in5', [])
        if density:
            metrics = self.calculate_metrics(density)
            print(f"\n6. DENSITY 4-IN-5 (n={metrics['sample_size']})")
            print(f"   Modifier: -1% density tax (ADJUSTED from -2%)")
            print(f"   Mean Error: {metrics['mean_error']:+.2f} pts")

            if abs(metrics['mean_error']) < 0.5:
                print(f"   ✅ WELL-CALIBRATED: Adjustment successful")
            elif metrics['mean_error'] > 0.5:
                print(f"   ⚠️  Still slight under-projection")
            else:
...
if __name__ == "__main__":
    print("\n🔬 Starting 60-Day Fatigue Backtest...")
    print(f"Window: {datetime.now() - timedelta(days=60):%Y-%m-%d} to {datetime.now():%Y-%m-%d}")

    backtest = FatigueBacktest(days=60)
