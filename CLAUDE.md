...
- **`player_game_logs.player_name` mixed accent encoding** — `players.name` stores accented names (`Luka Dončić`) but `player_game_logs.player_name` is non-accented for ~8 players (`Luka Doncic`). Direct JOIN on `players.name = player_game_logs.player_name` will silently undercount rows for these players. In compute scripts, always do a lookup: `SELECT player_name FROM player_game_logs WHERE player_name = ? LIMIT 1` with the canonical name first; if no rows, try the non-accented fallback via `unidecode(name)`. See `scripts/compute_empirical_modifiers.py` name resolution block.
- **Cross-table name JOIN with mixed accent encoding (general rule)** — `player_injuries.player_name` and other API-sourced tables may encode names differently from `players.name`. Never `JOIN ON table.player_name = players.name` across tables from different APIs — silently drops accented players. Correct pattern: two SELECTs + Python-side `unidecode(name).lower().strip()` comparison. See `utils/roster_validator._patch_injured_player_teams()` (Mar 29).
...
- **`ANALYSIS_PROTOCOL` vs `ANALYSIS_PROTOCOL_CURATION` — two separate constants** — `ANALYSIS_PROTOCOL` in `utils/claude_prompts.py` has 5+ callers including an explicit "rule 7" citation in `curate_plays.py` line 361. `ANALYSIS_PROTOCOL_CURATION` is the collapsed (113-token) version for Sonnet curation only (`scripts/curate_plays.py` system prompt). If editing curation data grounding rules, edit `ANALYSIS_PROTOCOL_CURATION`. If editing shared pipeline data rules, edit `ANALYSIS_PROTOCOL`. Never merge them — the multi-caller reference chain will break.
